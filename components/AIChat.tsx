...
const AIChat: React.FC<AIChatProps> = ({ onAddScenario, onAddScenarioAndViolation, uploadedFiles }) => {
  ...
  const handleSend = async () => {
    ...
    const auditKeywords = ['법인카드', '경비', '지출', '회계', '데이터 분석 감사', '부정 징후', '감사 분석', '위반 패턴', '거래처', '쪼개기 결제', '유흥업소', '가공 거래처', '심야 시간', '자택 인근', '중복 청구', '과도한 의존', '퇴직자', '계정 잔액', '클라우드 보안', '매출채권', '사적 사용'];
        ...
        if (findingsToDisplay.length > 0) {
          ...
          findingsToDisplay.forEach((finding, index) => {
            ...
            const newViolation: ViolationDetail = {
              ...
              controlPoint: 'AI Intelligence Audit',
        ...
        return (
          <div className="space-y-4">
            ...
            {findings.map((finding, idx) => (
              <div key={idx} className={`p-4 rounded-xl border shadow-sm ${finding.severity === 'High' ? 'bg-red-50 border-red-200' :
                ...
                <div className="flex justify-between items-center mb-2">
                  ...
                  <span className={`px-2 py-0.5 rounded-full text-xs font-bold ${finding.severity === 'High' ? 'bg-red-100 text-red-700' :
