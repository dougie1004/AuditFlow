...
const AuditReport: React.FC<AuditReportProps> = ({ scenarios, violations, isAuditComplete }) => {
    ...
    return (
        <div className="h-full flex flex-col items-center justify-center p-8 text-center bg-slate-50">
            <div className="bg-white p-12 rounded-3xl shadow-xl border border-slate-200 max-w-xl w-full">
                ...
                <h2 className="text-2xl font-bold text-slate-900 mb-3">보고서가 생성되지 않았습니다.</h2>
    ...
    switch (selectedTemplate) {
             ...
             <section>
              ...
              <div className="space-y-6">
                {[...violations]
                    ...
                    return (
                      <div key={idx} className={`bg-white border rounded-xl p-6 shadow-sm ${isNew ? 'border-red-300 ring-4 ring-red-50' : 'border-slate-200'}`}>
                         <div className="flex flex-col sm:flex-row justify-between items-start mb-4">
                            <div className="flex items-center gap-3">
                                ...
                                <h4 className="font-bold text-slate-900 text-lg">{violation.id}: {violation.controlPoint}</h4>
                            </div>
                            <span className="text-xs text-slate-500 font-mono">Ref: {violation.transactionInfo.id}</span>
                         </div>
      ...
      case 'improvement':
        return (
            <div className="space-y-8 animate-in fade-in duration-500">
                <section>
                    <h3 className="text-xl font-bold text-slate-900 mb-3 border-l-4 border-blue-600 pl-3">1. 전략적 개선 제언</h3>
          ...
          <div className="space-y-8 animate-in fade-in duration-500">
            <section>
              ...
              {newHighRiskScenarios.length > 0 && (
                <div className="mb-6 bg-red-50 border-l-4 border-red-500 p-5 rounded-r-xl shadow-sm">
                    <h4 className="flex items-center gap-2 text-red-800 font-bold mb-2 text-lg">
                        <AlertTriangle className="w-6 h-6"/> 신규 고위험 시나리오 탐지 (AI Urgent Findings)
                    </h4>
                    <p className="text-sm text-red-700 mb-4 leading-relaxed">AI 분석 결과 <strong>{newHighRiskScenarios.length}건</strong>의 심각한 통제 위반 징후가 추가 식별되었습니다.</p>
              ...
              <p className="text-slate-700 leading-relaxed text-justify">
                ...
                AuditFlow AI 엔진을 활용하여 총 <strong>{totalScenarios}개</strong>의 통제 시나리오를 점검하였습니다.
                <br/><br/>
            ...
            <section>
              ...
              <div className="bg-slate-50 p-4 sm:p-6 rounded-lg border border-slate-100">
                <ul className="list-disc list-inside space-y-2 text-slate-700">
                  ...
                  <li><strong>방법론:</strong> 정형 데이터 SQL 규칙 및 비정형 증빙 AI 교차 검증</li>
                </ul>
              </div>
            </section>
              ...
              <div className="overflow-x-auto border border-slate-200 rounded-lg">
                <table className="min-w-full divide-y divide-slate-200">
                  <thead className="bg-slate-50">
                    <tr>
                      <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase">영역</th>
                      <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase">적발 건수</th>
    ...
    <div className="h-full flex flex-col bg-slate-100 overflow-hidden">
      <div className="bg-white border-b border-slate-200 px-6 py-4 flex flex-col sm:flex-row justify-between items-center gap-4 print:hidden shrink-0">
        ...
        <div className="flex space-x-3 w-full sm:w-auto">
          <button className="flex-1 sm:flex-none flex items-center justify-center gap-2 px-4 py-2 bg-white border border-slate-200 rounded-lg text-slate-700 hover:bg-slate-50 font-medium text-sm shadow-sm" onClick={() => window.print()}>
      ...
      <div className="flex-1 flex overflow-hidden">
         <div className="w-80 bg-slate-50 border-r border-slate-200 p-4 flex-col gap-4 hidden lg:flex print:hidden overflow-y-auto">
            ...
            {TEMPLATES.map((template) => {
                ...
                return (
                    <button key={template.id} onClick={() => handleTemplateChange(template.id)} className={`text-left p-4 rounded-xl border transition-all duration-200 ${isSelected ? 'bg-white border-blue-600 shadow-md ring-1 ring-blue-600' : 'bg-white border-slate-200 hover:border-blue-300'}`}>
            ...
            <div className="max-w-5xl mx-auto bg-white shadow-lg rounded-xl min-h-[800px] p-8 sm:p-12 border border-slate-200 print:shadow-none print:border-none print:min-h-0">
                ...
                <div className="mt-16 pt-8 border-t border-slate-200 flex flex-col sm:flex-row justify-between gap-8">
                    <div>
                        ...
                        <p className="text-lg mt-4 font-serif italic">AuditFlow AI Auditor</p>
                    </div>
