        ...
        <div className="overflow-auto flex-1">
          <table className="w-full text-left border-collapse">
            <thead className="bg-slate-50 sticky top-0 z-10">
              <tr>
                <th className="p-4 text-xs font-semibold text-slate-500 uppercase tracking-wider border-b border-slate-200 hidden md:table-cell">ID</th>
                <th className="p-4 text-xs font-semibold text-slate-500 uppercase tracking-wider border-b border-slate-200">영역</th>
                <th className="p-4 text-xs font-semibold text-slate-500 uppercase tracking-wider border-b border-slate-200 w-full md:w-1/2">테스트 시나리오</th>
                <th className="p-4 text-xs font-semibold text-slate-500 uppercase tracking-wider border-b border-slate-200 hidden lg:table-cell">유형</th>
                <th className="p-4 text-xs font-semibold text-slate-500 uppercase tracking-wider border-b border-slate-200 hidden xl:table-cell">증빙</th>
            ...
            <tbody className="divide-y divide-slate-100">
              {filteredScenarios.map((scenario) => (
                <tr key={scenario.id} className="hover:bg-slate-50 transition-colors group">
                  <td className="p-4 text-sm font-mono text-slate-500 hidden md:table-cell">{scenario.id}</td>
                  ...
                  <td className="p-4">
                    ...
                    <p className="text-xs text-slate-500 mt-1 hidden md:block">{scenario.detailedDescription}</p>
                     <button
                       onClick={() => setSelectedScenario(getScenarioDetail(scenario))}
                       className="flex xl:hidden items-center mt-2 gap-1 text-slate-600 hover:text-blue-600 hover:bg-blue-50 transition-colors text-xs font-medium border border-slate-200 hover:border-blue-300 rounded px-2 py-1 bg-white w-fit shadow-sm"
                     >
      ...
      {selectedScenario && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-5xl bg-white rounded-2xl shadow-2xl overflow-hidden max-h-[90vh] flex flex-col">
            ...
            {/* Modal Header */}
            <div className="flex items-start justify-between px-4 py-4 sm:px-8 sm:py-5 border-b border-slate-100 bg-slate-50/50">
              <div className="flex-1">
                 ...
                 <h2 className="text-lg sm:text-xl font-bold text-slate-900">{selectedScenario.controlPoint}</h2>
              </div>
              <button 
                onClick={() => setSelectedScenario(null)}
                className="p-2 rounded-full hover:bg-slate-200 transition-colors text-slate-500 ml-4"
              >
                <X className="w-6 h-6" />
              </button>
            </div>
            ...
            {/* Modal Content */}
            <div className="p-4 sm:p-6 lg:p-8 overflow-y-auto bg-slate-50/30">
              
              {/* AI Analysis Box */}
              <div className={`border rounded-xl p-4 sm:p-6 mb-6 sm:mb-8 relative overflow-hidden ${
                ...
                <div className="absolute top-0 right-0 p-4 opacity-10">
                  <Eye className={`w-16 h-16 sm:w-24 sm:h-24 ${selectedScenario.riskLevel === 'Low' ? 'text-emerald-600' : 'text-red-600'}`} />
                </div>
                ...
                <div className="bg-white p-4 sm:p-6 rounded-xl border border-slate-200 shadow-sm">
                  ...
                  <div className="space-y-4">
                    <div>
                      ...
                      <p className="text-base sm:text-lg font-mono font-medium text-slate-900 break-all">{selectedScenario.transactionInfo.id}</p>
                    </div>
                    <div>
