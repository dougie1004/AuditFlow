...
try {
    ...
    // It ends approx 10 lines after anchor.
    // We scan for `}` at correct nesting level? No, assume code is formatted enough.
    // Or just search for the specific end pattern `Ok(serde_json::to_value(payload).map_err(|e| e.to_string())?)\n}`
    // Let's be a bit more robust: count braces? Too complex.
    // Just manual visual check says it's near line 2412.

    // Actually, duplicated content is AFTER the anchor.
    // So if we truncate at the END of anchor, we are good.

    // Find the closing brace `}` of the Anchor function.
    // Since I know the content of the Anchor function end:
    const anchorEndPattern = 'Ok(serde_json::to_value(payload).map_err(|e| e.to_string())?)\n}';
    // Note: newlines might vary (\r\n vs \n).
    // I'll search for `Ok(serde_json::to_value(payload).map_err(|e| e.to_string())?)` then find next `}`.

    const patternIndex = content.indexOf('Ok(serde_json::to_value(payload).map_err(|e| e.to_string())?)', lastAnchorIndex);
    if (patternIndex === -1) {
        console.log('Error: Could not find anchor end pattern!');
        // Fallback: search for `}` after anchor
        // ...
        process.exit(1);
    }
